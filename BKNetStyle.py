...
def batch_norm(x, n_out, phase_train=True, scope='bn'):
    with tf.compat.v1.variable_scope(scope):
        ...
        normed = tf.compat.v1.nn.batch_normalization(x, mean, var, beta, gamma, 1e-3)
    return normed

# Input
def Input():
    x = tf.compat.v1.placeholder(tf.float32, [None, IMG_SIZE, IMG_SIZE, 1])
...
def _conv(name, x, filter_size, in_filters, out_filters, strides):
    with tf.compat.v1.variable_scope(name):
        ...
        filter = tf.compat.v1.get_variable('DW', [filter_size, filter_size, in_filters, out_filters], tf.float32, tf.random_normal_initializer(stddev=WEIGHT_INIT))
...
def _FC(name, x, out_dim, keep_rate, activation='relu'):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        W = tf.compat.v1.get_variable('DW', [x.get_shape()[1], out_dim], initializer=tf.random_normal_initializer(stddev=WEIGHT_INIT))
        b = tf.compat.v1.get_variable('bias', [out_dim], initializer=tf.constant_initializer())
...
# Caculate learning rate
def train_op(loss, global_step):
    learning_rate = tf.compat.v1.train.exponential_decay(INIT_LR, global_step, DECAY_STEP, DECAY_LR_RATE, staircase=True)
